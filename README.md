...
- **[code-review](./skills/engineering/code-review/SKILL.md)** — Three-axis review of the diff since a fixed point: **Features** (does it faithfully implement the originating issue/PRD?), **Structure** (repo conventions plus a Fowler smell baseline), and **Design** (sound module shape — deep interfaces, clean seams), each run as an adversarial review then a verification pass so none pollute each other.
